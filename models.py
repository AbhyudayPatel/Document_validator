...
class DocumentRequest(BaseModel):
    ...
    document_text: str = Field(
        ...,
        description="The raw text content of the insurance document.",
        min_length=1
    )
...
class ExtractedData(BaseModel):
    ...
    policy_number: Optional[str] = Field(
        None,
        description="The unique policy reference number"
    )
    vessel_name: Optional[str] = Field(
        None,
        description="The name of the insured vessel"
    )
    ...
    insured_value: Optional[int] = Field(
        None,
        description="The total insured value in USD (as integer)"
    )
...
class ValidationResult(BaseModel):
    ...
    rule: str = Field(
        ...,
        description="The name of the validation rule being checked"
    )
    status: str = Field(
        ...,
