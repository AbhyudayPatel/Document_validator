...
def test_validation_rules():
    ...
    def test_date_same():
        ...
        assert result.status == "FAIL", f"Same dates should fail"
    ...
    for filename, should_pass, description in test_cases:
        def test_doc():
            ...
            extracted = ExtractedData(
                ...
                policy_start_date=date.fromisoformat(ai_result.policy_start_date) if ai_result.policy_start_date else None,
                policy_end_date=date.fromisoformat(ai_result.policy_end_date) if ai_result.policy_end_date else None,
            ...
            if should_pass:
                assert all_passed, f"Expected all validations to pass but {4-passed_count} failed"
            else:
                assert not all_passed, f"Expected some validations to fail but all passed"
...
def test_edge_cases():
    ...
    vessels = ["MV Neptune", "Oceanic Voyager", "Starlight Carrier", 
