...
class DocumentValidator:
    ...
    def _load_valid_vessels(self) -> list[str]:
        ...
        try:
            ...
            raise json.JSONDecodeError(
                f"Invalid JSON in vessels file: {str(e)}", e.doc, e.pos
            )
    ...
    def validate_date_consistency(
        ...
        if end_date > start_date:
            return ValidationResult(
                rule=rule, status="PASS", message="Policy end date is after start date."
            )
        else:
            return ValidationResult(
                rule=rule,
    ...
    def validate_insured_value(self, insured_value: Optional[int]) -> ValidationResult:
        ...
        if insured_value is None:
            return ValidationResult(
                rule=rule, status="FAIL", message="Insured value is missing."
            )

        if insured_value > 0:
            return ValidationResult(
                rule=rule, status="PASS", message="Insured value is valid."
            )
        else:
            return ValidationResult(
                rule=rule,
    ...
    def validate_vessel_name(self, vessel_name: Optional[str]) -> ValidationResult:
        ...
        if vessel_name is None or vessel_name.strip() == "":
            return ValidationResult(
                rule=rule, status="FAIL", message="Vessel name is missing."
            )
    ...
    def validate_policy_number(self, policy_number: Optional[str]) -> ValidationResult:
        ...
        if policy_number is None or policy_number.strip() == "":
            return ValidationResult(
                rule=rule, status="FAIL", message="Policy number is missing."
            )

        return ValidationResult(
            rule=rule, status="PASS", message="Policy number is present."
        )
    ...
    def validate_all(self, extracted_data: ExtractedData) -> list[ValidationResult]:
        ...
        results.append(
            self.validate_date_consistency(
                extracted_data.policy_start_date, extracted_data.policy_end_date
            )
        )

        results.append(self.validate_insured_value(extracted_data.insured_value))

        results.append(self.validate_vessel_name(extracted_data.vessel_name))

        results.append(self.validate_policy_number(extracted_data.policy_number))
